...
## 3C3U-金萝卜-MCC命令行客户端使用文档 [​](#_3c3u-金萝卜-mcc命令行客户端使用文档)
...
<!-- valaxy-encrypt-start:gxzh123456 -->

> ⚠️ **警告：账号请勿告知他人，以免泄露**

加粗表示该账户密码已经共享出来。

| 分类           | 小朋友1239187806 | 嗯呀2705249150 | !!!893466334 | 薪2745223423 | skr3663455832 |
| -------------- | ---------------- | -------------- | ------------ | ------------ | ------------- |
| 基地 3         | liveou           | 23571113       | slimy_       | Xinxu_xx     | skr           |
|                | WDDWWB           |                | hj123        |              |               |
|                | superman         |                |              |              |               |
|                | buu              |                |              |              |               |
|                | HEBAIJU          |                |              |              |               |
| 基地 2         | nike             | 34Y667NUY      | hj1          |              |               |
|                | WANGBIN          |                |              |              |               |
| 基地 1         | belle            |                |              |              | plghkj        |
|                | panda            |                |              |              |               |
|                | qsb              |                |              |              |               |
...
经过第一次的使用MCC客户端以及自己搭建服务器节点后，目前发现很多问题。

情况如下

之前未长期续费的原因：

1.使用“润雨云”提供的面板服务器，我发现了我会经常容易忘记签到，导致一个月漏掉了很多天。从而导致不能及时给服务器续费。故后面没有使用润雨云的面板服务器。

2.同时的方案还有：购买nat便宜服务器，续费价格3元一个月。以及连续续费两个月，随后因为**过年**从而导致忘记续费，且服务器商家会将服务器关机，然后需要我手动打开，才能连接成功，故已经抛弃当前方案。

当前情况：

以上两种使用服务器方式均已失效，没能实现长期续费……

现在解决办法：

经过之前搭建服务器的经验，结合当前比较火的**飞牛NAS**其中可以使用Docker来安装MCSM面板从而来使用MCC客户端。综合考虑之后，我决定购买一台小主机。要求是功耗低，能够长时间开机，可以远程管理。经过查找后选定了一个J1900cpu的瘦客户端来作为我的服务器。

在网络方面我选择了使用frp的ipv4穿透+ipv6直连。

## MCSManager Panel [​](#mcsmanager-panel)

使用指南：

### 1.登陆 [​](#_1-登陆)

在浏览器地址栏输入**f.hebaiju.cn:23333**及可以访问到web控制面板。随后输入账号密码即可

![MCSM登陆面板0001](https://pic1.imgdb.cn/item/699d62d501b9a5550a5438c9.png)

### 2.应用实例 [​](#_2-应用实例)

因为服务器的连接采用frp端口映射和ipv6直连。

①若你有ipv6则可以使用ipv6节点来更快的进行访问，若看不明白则选择ipv4默认节点即可。两个节点只有线路不同，内容均为相同。

②随后既可以进入例如jb\_19CM该机器人终端进行管理。

另外：该web面板属于“傻瓜式”操作逻辑，故此随后将不会对简单的操作进行解释，可以自行研究。

![PixPin_2026-02-24_16-48-41.png](https://pic1.imgdb.cn/item/699d65e201b9a5550a5438fb.png)

#### ①应用实例讲解 [​](#_1应用实例讲解)

进入演示的jb\_19CM中后会有如下图界面，在点击开启实例之后会有几率在实例进程和应用进程中间点击如①关闭按钮可以会卡在正在停止中，也就是将关闭指令没有发送到MCC机器人程序当中，而实例误以为已经发送成功指令，从而一直卡在②的终止处。从而不能完整的保存配置文件，有几率下次不能成功启动实例。

还有一种情况为**关闭实例命令**错误，如下图④处。MCC机器人的关机指令为`/quit`而默认关闭容器的命令为`Ctrl+C` / `^C`。导致MCC机器人不能识别

解决办法为：点击③终端设置，将命令修改为`/quit`。注意若要关闭web窗口任然实现容器运行需要**关闭仿真终端**开关

![PixPin_2026-02-24_18-15-15.png](https://pic1.imgdb.cn/item/699d7a4101b9a5550a549bce.png)

### 3.关于实例的高级配置 [​](#_3-关于实例的高级配置)

只有管理员可以修改和管理

#### ①启动命令：`./mcc jb_19CM` [​](#_1启动命令-mcc-jb-19cm)

![PixPin_2026-02-24_18-35-36.png](https://pic1.imgdb.cn/item/699d7f0001b9a5550a549c1b.png)
...
更多启动命令的配置参考 [Minecraft 命令行客户端](https://mccteam.github.io/l10n/zh-Hans/guide/usage.html#%E5%91%BD%E4%BB%A4%E8%A1%8C%E7%94%A8%E6%B3%95)。
...
现在实例采取[Docker 容器的形式](https://www.runoob.com/docker/docker-intro.html)来运行以此来保护主机的安全性。

可以设置的内容不多，按照图片中设置即可

![PixPin_2026-02-24_20-05-59.png](https://pic1.imgdb.cn/item/699d943101b9a5550a550751.png)

其中值得注意的是**环境变量**可以向容器内传递参数，如下图中的参数就能将容器的系统语言修改为中文。从而使MCC获取到的参数修改为中文。该方法在其他的地方仍然适用。

![PixPin_2026-02-24_20-09-10.png](https://pic1.imgdb.cn/item/699d94eb01b9a5550a55075b.png)

④容器限制界面暂时没有东西需要修改

![PixPin_2026-02-24_20-01-01.png](https://pic1.imgdb.cn/item/699d931701b9a5550a55073e.png)
...
- log.txt：这个是服务器消息日志文件。用于记录聊天成就等信息。可以在配置文件里面开启或者关闭，或者
- MinecraftClient-20250522-285-linux-x64：机器人本体。文件名称可以修改。
...
![PixPin_2026-02-24_20-31-14.png](https://pic1.imgdb.cn/item/699d9a1701b9a5550a5507aa.png)
