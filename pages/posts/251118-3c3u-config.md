...
## 3C3U-MCC命令行客户端 [​](#_3c3u-mcc命令行客户端)

> 本篇文章将介绍金萝卜使用的命令行客户端，后台控制，链接等。

目前本基地有正式成员4人。

空闲用于挂机账号多个，详情见[附](#%E9%99%84)。

目前本人[HEBAIJU](https://www.hebaiju.cn/)购买服务器用来运行机器人，在想办法节约成本。故会根据服务器的价格来随时更换服务器。你可以点击下面链接进行跳转

| 第一台 | 第二台 |
| --- | --- |
| [付费服🎮](http://f.hebaiju.cn:23333/) | [签到免费服🎮](https://mcsm.rainyun.com/) |

## 当前情况 [​](#当前情况)

当前可用服务器共计2台，都使用[MCSManager Panel](https://docs.mcsmanager.com/)管理。但是因为不同的原因当前只能使用两台。且应为3C的限制一个IP只能同时登陆3个账号！

第一台：[MCSManager Panel](http://61.174.242.144:23333/) 。**完全能用**。此服务器为主服务器，目前正在使用，

**第一台**服务器内有多个机器人，每个机器人都有一个独立的账号。使用管理账号均能使用。但是注意配置文件，稍后我会详细说明。有多个终端，但是最多在3C中运行3个挂机账号，应为3C限制每个ip登陆3个账号。需要管理员账号给我发信息。

第二台：[MCSManager Panel](https://mcsm.rainyun.com/)。**一般能用**。此服务器可以使用，应为是免费的服务器面板，故此只能运行一个机器人，但是服务器出于大厂非常稳定。

**第二台**服务器是来自[雨云](https://app.rainyun.com/)。的免费面板服，原先的作用是来开mc服务器，但是我的修改后成为了一个挂机的服务器，但是只有一个终端，所以只能运行一个机器人。
...
| 是否能用 | yes | yes |
| 可以在线多少 | 3 | 1 |
| 价格 | HEBAIJU在续费 | 免费，但是需要[签到](https://app.rainyun.com/account/reward/earn#) |
| 服务器位置 |  | 宿迁 |
...
[第一台服务器MCSManager Panel](http://61.174.242.144:23333/) 。**完全能用**。我拥有此服务器的管理员权限，面板我也能给与管理员权限。服务器将由我进行续费，和维护。你们不用操心。此服务器为主服务器，目前正在使用，相对来书比较理想的服务器，但是他的服务商**比较容易跑路**，因此我可能会随时更换掉，可以访问当前页面进行跳转。

### 第一步：登陆 [​](#第一步-登陆)

点击当前页面上[这个链接](http://61.174.242.144:23333/)进入服务器控制台:

![1登陆](https://pic1.imgdb.cn/item/699c8f687aea005f3831d0fe.png)
...
![选择实例1](https://pic1.imgdb.cn/item/699c8f687aea005f3831d100.png)

如图所示，你可以随意去点击，进入任意一个实例终端

![选择实例2](https://i1.hdslb.com/bfs/new_dyn/7d4ae43e5414943f248da5c43513977b3546648970005086.png@1192w.avif)

一般情况下你只用点击这里的开启按钮，或者关闭按钮，因为里面的所有配置我已经配置好了。下面红色方块内的为一些其他的功能，可以管理文件，对终端进行设置，或者一些其他的……

### 第三步：文件管理 [​](#第三步-文件管理)

点击上图中下方的**文件管理**按钮，文件管理里面可以查看例如：日志文件，配置文件等。

![3文件管理](https://i1.hdslb.com/bfs/new_dyn/f657afabeb2a2e535ff31dc85c2a6ecc3546648970005086.png@1192w.avif)

如图所示其中有很多文件，我将一一介绍：

- Sentry：这个是用于机器人的UUID，不用管
- lang：这个是服务器的翻译配置文件，里面有一个zh\_cn.json。
...
- MinecraftClient.ini：机器人配置文件，优先读取，可以修改，但是记得备份。点击[Minecraft 命令行客户端](https://mccteam.github.io/l10n/zh-Hans/guide/)查看详细配置。
- core：这个我也不知道是干什么的。
- 启动脚本(可修改).sh：启动文件，切记不要改名字。

  启动脚本解释：./MinecraftClient-20250522-285-linux-x64 jb\_19CM

  ”./“执行当前目录下的MinecraftClient-20250522-285-linux-x64文件，"jb\_19CM"启动参数，需要进入游戏的用户名。
...
这里**最重要**的是**启动命令**，等同于第三步的最后一个文件命令，

```
./MinecraftClient-20250522-285-linux-x64 WDddl
```

含义：“./”执行当前目录下的“MinecraftClient-20250522-285-linux-x64文件”，启动用户名是"WDddl"。一般不用修改。

注意

记得在文件管理里面给MinecraftClient-20250522-285-linux-x64添加可执行权限

![1.4实例设置](https://i1.hdslb.com/bfs/new_dyn/087edab28c7c7500980e0a09bd0bd9363546648970005086.png@1192w.avif)

## 第二台服务器 [​](#第二台服务器)

其中最重要的是**第二台**服务器，**第二台**服务器是来自[雨云](https://app.rainyun.com/)的免费游戏面板服，原先的作用是来开mc服务器，但是我的修改后成为了一个可以挂机的服务器，但是只有一个Docker终端，所以只能运行一个机器人。

它出于服务商雨云，基本不会跑路。

[雨云](https://app.rainyun.com/)此服务器采用签到付费，[签到](https://app.rainyun.com/account/reward/earn#)每天可以获得500积分，服务器续费一个月需要10000积分。所有需要在一个月内至少签到**20天**。

> 下面是签到的图，如果我漏签了麻烦帮忙签到一下。

![2.2.1签到](https://i1.hdslb.com/bfs/new_dyn/ced5b9e5cb57de87664796c7a18a326c3546648970005086.png@1192w.avif)

### 第一步：登陆 [​](#第一步-登陆-1)

[点击该链接](https://app.rainyun.com/account/reward/earn#)直接进入[雨云](https://app.rainyun.com/)的官网。登陆后自动跳转到签到页面。如下图是登陆页面。

![2.1.1登陆](http://i0.hdslb.com/bfs/new_dyn/160715e364b4e41c913aeb9d976b9e1d3546648970005086.png)
...
![2.2.1签到](https://preview.cloud.189.cn/image/imageAction?param=6A3557059DE00AA6038845EB8F160AE566224E229A2887F21CDB15681960EE9D9938F998DD0E5E7833B6C66FD1C6079F0DE5D06E550E3F0F314375BD41D39603B153C51B48048FF6D3E6B3D5DBC937DE154E92A6F37B3373526368C5BF667E20CBD58CBD2FB93A8C985FB8EBE3A780BC58644F3F)

### 第三步：人机验证 [​](#第三步-人机验证)

点击（**领取奖励**）按钮则会弹出腾讯公司的人机验证（由于此人机校验码的难度太高，我没能实现自动验证登陆）。

![2.3.1人机验证](https://preview.cloud.189.cn/image/imageAction?param=3EE3E75B7D47C26B9EAD54C4B4B9B94DD97B456ED007EDCA4ABDBF4968DF5FC7BA0FF05022DAE06BEA2AD8603D6E7EE5E85A34D5523933D622A8C3763E5C59C71A9FF1E34602ED1372C25E3726F2355129CE71717809BDD98E039D2819E8E8FCBCFFE959AB83044A68CF4952A78805809D6FE7DA)

### 第四步：积分反馈 [​](#第四步-积分反馈)

完成人机验证，随后就可以看到下图红框中的积分增加了**500**分。领取奖励的按钮成为灰色则签到成功。

![2.4.1积分](https://preview.cloud.189.cn/image/imageAction?param=0A4E1DB6D4E9977B591A07FDE89819ABA19845390A5410A8E56BE2131AECF0B24F33DCBD98C10B6EA43B5D88EDC4F8767837CE1E1F0A393A031DB6401E1E5C0364A6C1C3222DDA7A9CA95B43E08B1BC499F5C541FFD21D64459A1067391AE16A6B6F18B244CEDF07992D8CA8D972E9F66D155F55)![2.4.2积分](https://preview.cloud.189.cn/image/imageAction?param=3CB704D277326C3A802C8B046752EB7FCE600EF7F4B8817C176A52FF1343214063C8CD50C295BEDB02403FBF091F3DC3233670F2807F7B8929BC206F0F043C0155405E32A065E90D6385CC50C2A3A4753597A4A88A4A0D9422459A87E94F1E57B86E6D28EA174ACCF808FC5AAB29D0DF5012BD57)

### 第五步：续费 [​](#第五步-续费)

之后就可以点击[免费续费](https://app.rainyun.com/apps/rgs/65489/detail)来对服务器进行续费。

橙色框内有续费的要求\*（有些要求是正常的，毕竟免费的是最贵的嘛！）\*

![2.5.1续费](https://preview.cloud.189.cn/image/imageAction?param=BE475E65CBEE679A5723881EB16ACD4803EA8ABA08ED2F6A2F2E12923B301C0411062F2AA026F1DEBA741F8B268EF75DAC9C189002DAF4ED1DE1582BCABD668F4EEF65FBADE06E06908CD21642EC89B82F4B04F2305EFC1030007C290B37D1D46A4CF830FD9046746CB138C211DD95D19AF9D08A)

### 第六步：获取密码 [​](#第六步-获取密码)

密码的获取：[下滑](https://app.rainyun.com/apps/rgs/list)页面可以看如下图的的管理面板窗口，在这里面来获取你的账号[密码](https://app.rainyun.com/apps/rgs/list)在下一步进行面版的登陆管理。

![2.6.1密码](https://preview.cloud.189.cn/image/imageAction?param=EE84220E84A2F492FBCC82E8F2FE24062DC31ECCA1574C3FC598CFBC21E8CC7A4FEF642200CD2299AB9545879A611E3AA32B92BBA2EDE4C03AB5B806857063C415943F3CD914225CC6A3307ABF289FAF3B66601F76D60D10CCE44CCC7EDF7845F0F17F5931945A7C917D7876E3A0B7DACF02EE27)

### 第七步：登陆面板 [​](#第七步-登陆面板)

可以如上图所示点击[（进入控制台(MCSM)）](https://mcsm.rainyun.com/#/login)，再输入上图获取的密码，对面板服务器进行管理。如下图所示。

![2.7.1面板登陆](https://preview.cloud.189.cn/image/imageAction?param=D11FA923A8BD0E3F4E6E18D2A21BAE282265450E9043A12CCE3771C73FCC55E5BA58FC02C4590A4B45FF63C8443FFCB3D8514E66E16EEC5FC9B45C0E1997EA62BDB8E79F3B41F014200B8CFD29A1AD4718DB16525614EE18E72B80CFB166E95B41031FD26D205F266D42B44BCF861B23F1D57B31)

### 第八步：登陆控制台 [​](#第八步-登陆控制台)

点击登陆按钮后，如下图页面，再点击进入控制台。

![2.8.1管理面板](https://preview.cloud.189.cn/image/imageAction?param=13FF12074406559384EAA4B016654DF5999F12500DF2BFBCB06AB1BDB66AD1E4604AF9ED8428AFC8FCA957EB66F4FD7A18B535041182F7536256765895272E2E7D63F039D4C6A8F49D71216BF0D769A75AE0196EBF34F4A5C408C6469A65D9C7592E72A5615DBABA0E211F609DAA19B653DB4647)

### 第九步：控制台管理 [​](#第九步-控制台管理)

这个时候你就完全登陆进来了，里面所有的东西都可以操作，服务器的开启或者关闭等等一切。

![2.9.1控制台](https://preview.cloud.189.cn/image/imageAction?param=B1CC1D9D93500ECF0EEAA21A3A327EF9DBE8B079DB62799A1780747FFC8D52A5BBCAEFB40C2CB8FA81733776541A3C7D7840A79E2DC9DA99D80A11F163834E3C6771E956388E33491421B954F5921B0B5CC4948A5F9037652EA770A8F2FC90D439759E0E5E75C82F1102DEF1C6AFB857C29ACAA4)

一般情况下点击关闭按钮就能直接关闭，如果遇到故障则点击终止按钮。

### 第十步：文件管理 [​](#第十步-文件管理)

在下方有**文件管理**按钮，里面可以查看例如：日志文件，配置文件等。

![2.10.1文件管理](https://preview.cloud.189.cn/image/imageAction?param=92780F572AFBD1F21B30D29EBD2A56F0A63B6C86427B3631AA3254FF3A88F0A41B4E44B4C6C109794841C370530E9F2C9C9B214DD1A2EC0E069915A1BADCF9ECCD59EAA333A2316958B3B0E3D62EC89BEBBD9635C54145F5AD781841D3FCD1520405D28FF6D64CD6513A8864F5A7B7AD9099069D)
...
综上所述，目前共有2台服务器，我会尽力去续费，愿游玩的开心。最后感谢我的小伙伴们。

## 附： [​](#附)

### 挂机机器人详细配置 [​](#_2-挂机机器人详细配置)

你可以访问[Minecraft 命令行客户端](https://mccteam.github.io/l10n/zh-Hans/guide/)来查看更加详细的配置。
